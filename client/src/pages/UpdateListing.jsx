...
export default function UpdateListing() {
  ...
  const handleChange =(e)=>{
       ...
       if(e.target.id==='offers'|| e.target.id==='parking' || e.target.id==='furnished' ){
        setFormData({
          ...formData,
  ...
  return (  
    ...
    <h1 className='text-3xl text-center my-4 font-semibold'>Update a Listing</h1>
       ...
       <div className='flex flex-col gap-4 flex-1'>
        ...
        <div className='flex gap-6 flex-wrap'>
         ...
         onChange={handleChange}   checked={formData.offers} />
            <span>Offers</span>
          </div>
          <div className='flex gap-2'>
        ...
        <div className='flex flex-wrap gap-6'>
          <div className='flex gap-2 items-center'>
            <input type='number' id='bedrooms' max='10' min='1'  className='rounded-md border p-3 border-slate-300' required
          ...
          <div className='flex gap-2 items-center'>
            <input type='number' id='regularPrice'  className='rounded-md border p-3 border-slate-300' required
                  min="500"  max="10000000"     onChange={handleChange} value={formData.regularPrice}  />
        ...
        {formData.offers &&(
            ...
            <input type='number' id='discountPrice' minLength='1' maxLength='10' className='rounded-md border p-3 border-slate-300' required
             min="0"  max="10000000"  onChange={handleChange} value={formData.discountPrice}  />
            <div className='flex flex-col items-center'>
            <p>Discount Price</p>
            <span className='text-xs'>INR / Day</span>
            </div>
       ...
       <div className='flex gap-4'>
        ...
        multiple className=' p-3 rounded-md w-full border border-gray-300'
        onChange={(e)=>setFile(e.target.files)} />
        <button disabled={uploading} type="button" className='uppercase text-green-500 
        rounded-md p-3 border border-gray-700 hover:shadow-lg disabled:opacity-75'
        onClick={handleImage} >{uploading ? "Uploading":"Upload"}</button> 
       </div>
       ...
       <button disabled={loading||uploading} className='p-3 uppercase bg-slate-500 text-white rounded-md hover:opacity-90 disabled:opacity-75'>{loading ? 'updating...':'Update Listing'}</button>
